	...
	public void run() {
		while (!workList.isEmpty()) {
			Vertex u = null;
	   		try{
				u = workList.remove(0);
			} catch(IndexOutOfBoundsException e) {} //Skip someone did steal it
			if(u != null) {
				u.setListed(false);
...
class FastDataflow {
	...
	public static void liveness(Vertex vertex[], int nthread)
	{
		...
		// Work for workers finished
		workerThreads.forEach(t -> {try{t.join();}catch(Exception e){}});
